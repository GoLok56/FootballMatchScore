...
class FavoriteTeamCache(private val db: DatabaseHelper) : Cache<TeamData> {
    ...
    override fun isExpired(item: TeamData) = false

    override fun put(item: TeamData) = db.use {
        insert(
            TeamData.TABLE_TEAM_FAVORITE,
    ...
    override fun get(id: String) = db.use {
        select(TeamData.TABLE_TEAM_FAVORITE)
    ...
    override fun clear() {
        db.use { delete(TeamData.TABLE_TEAM_FAVORITE) }
    }
